/* compass-systemik.js — das Wirkungsbild, das neben John und Madeleine mitläuft (10.09.2026)
   ...
   nicht an. Die Karte von John (#stapelBody) und die von Madeleine (#madBody) bekommen einen zweiten
   ...
   Woher die Daten kommen: NICHT aus dem Text geraten. `POST /api/systembild` legt John (bzw. dem
...
(function(){
  ...
  const FELDER={
    mad:  { feld:'madBody',    karte:'madKachel',  quelle:'beraterrunde', wer:'Madeleine & John' },
    john: { feld:'stapelBody', karte:'johnKachel', quelle:'stapel',       wer:'John' }
  };
  const S={};
  ...
  function quelleJohn(){
    ...
    const zeilen=offen.map((p,i)=>`${i+1}. ${p.titel} — ${p.satz||''}${p.aktion?` [Johns Vorschlag: ${p.aktion.label||p.aktion.art}]`:''}`);
    const text='Johns Stapel für heute, in dieser Reihenfolge:\n'+zeilen.join('\n');
    if(text.trim().length<80) return null;
  ...
  async function holen(art, frisch){
    ...
    if(!q){
      s.bild=null; s.fehler=''; s.leer = art==='mad'
        ? 'Noch keine Beraterrunde. Sobald John und Madeleine beraten haben, steht hier das Wirkungsbild dazu.'
  ...
  /* Zwei Tasten in der Sprache der Sache, nicht „Ja"/„Nein" — und sie tun wirklich etwas:
     bei Madeleine geht die Entscheidung in die Beraterrunde (John antwortet darauf), bei John
  ...
  function ctaHtml(art,b){
    ...
    return `<div class="syscta">`+
      ...
      `<div class="sysfuss">${art==='mad'?'Geht in john/coaching/beraterrunde.md — John und Madeleine lesen es beide.':'Zustimmen löst Johns Vorschlag aus; ablehnen legt den Punkt zurück und öffnet das Gespräch.'}</div>`+
    `</div>`;
  }
  ...
  function belohnen(art, xp, wahl){
    ...
    /* Nach einer Entscheidung ist der Text von gestern: neu holen, aber erst, wenn der Server
       mit der Antwort fertig ist — bei Madeleine schreibt John noch seinen Schluss. */
  ...
  function start(){
    ...
    /* Erst der Cockpit-Aufbau, dann wir: Johns Stapel und die Beraterrunde sollen zuerst laden —
